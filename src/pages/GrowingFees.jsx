...
function StatusBadge({ status }) {
  ...
  const labels = {
    paid:    `✓ ${t('growingFees.status.paid')}`,
    partial: t('growingFees.status.partial'),
    pending: t('growingFees.status.pending'),
  }
  return (
...
function GiveAdvanceModal({ farms, onClose, onSaved }) {
  ...
  useEffect(() => {
    supabase.from('accounts').select('id, name, type').eq('organization_id', organization.id).eq('is_active', true).order('created_at')
      .then(({ data }) => {
        const accs = data || []
        setAccounts(accs)
  ...
  useEffect(() => {
    ...
    supabase.from('batches').select('id, start_date, chick_count').eq('organization_id', organization.id).eq('farm_id', form.farm_id).eq('status', 'active').order('start_date')
      .then(({ data }) => {
        const bs = data || []
        setActiveBatches(bs)
  ...
  async function handleSubmit(ev) {
    ...
    const { data: currentBatch } = await supabase.from('batches').select('total_advances').eq('organization_id', organization.id).eq('id', form.batch_id).single()
    await supabase.from('batches').update({
      total_advances: Number(currentBatch?.total_advances || 0) + amt,
    }).eq('organization_id', organization.id).eq('id', form.batch_id)

    const selectedBatch = activeBatches.find(b => b.id === form.batch_id)
    const batchStartDate = selectedBatch ? formatDate(selectedBatch.start_date + 'T12:00:00', i18n.language) : ''
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Farm */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('reports.farm')} *</label>
            <select required value={form.farm_id} onChange={set('farm_id')}
              ...
              <option value="">{t('growingFees.selectFarm')}</option>
          ...
          {form.farm_id && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('growingFees.batch')} *</label>
              {activeBatches.length === 0 ? (
                ...
                <div className="rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-800 font-medium">
                  {t('growingFees.batchLabel')} {formatDate(activeBatches[0].start_date + 'T12:00:00', i18n.language)} — {t('growingFees.day')} {Math.floor((Date.now() - new Date(activeBatches[0].start_date + 'T00:00:00')) / 86400000)} — {Number(activeBatches[0].chick_count).toLocaleString('en-IN')} {t('growingFees.chicks')}
                </div>
              ) : (
                <select required value={form.batch_id} onChange={set('batch_id')}
                  ...
                  {activeBatches.map(b => {
                    ...
                    return <option key={b.id} value={b.id}>{t('growingFees.batchLabel')} {sd} — {t('growingFees.day')} {day} — {Number(b.chick_count).toLocaleString('en-IN')} {t('growingFees.chicks')}</option>
                  })}
                </select>
              )}
            </div>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button type="submit" disabled={saving || (form.farm_id && activeBatches.length === 0)}
              ...
              {saving ? t('common.loading') : t('growingFees.giveAdvance')}
            </button>
          </div>
        </form>
      </div>
...
function RecordPaymentModal({ initialFarmId, farmGroups, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-semibold text-gray-800">{t('growingFees.recordPayment')}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none">&times;</button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {farmGroups.map(g => {
                ...
                return (
                  <option key={g.farm_id} value={g.farm_id}>
                    {g.farm_name} — {t('growingFees.outstanding')}: {formatCurrency(outstanding)}
                  </option>
                )
              })}
            </select>
          ...
          {selectedGroup && pendingEntries.length > 0 && (
            <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">{t('growingFees.pendingFeeRecords')}</p>
                  ...
                  <div key={e.id} className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-700 truncate">
                        ...
                        {e.batch_start_date ? ` — Batch ${formatDate(e.batch_start_date, i18n.language)}` : ''}
                      </p>
                      <p className="text-xs text-gray-400">
                        {t('growingFees.ratePerKg')} {formatCurrency(e.rate_per_kg)}/kg · {Number(e.total_sale_kg).toLocaleString('en-IN')} kg
                      </p>
                    </div>
              ...
              <div className="border-t border-gray-200 mt-3 pt-3 flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-600">{t('growingFees.outstanding')}</span>
          ...
          <div>
            ...
            {form.amount && totalOutstanding > 0 && parseFloat(form.amount) > totalOutstanding + 0.01 && (
              <p className="text-xs text-orange-600 mt-1">{t('growingFees.exceedsOutstanding')}</p>
            )}
          </div>
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              {saving ? t('common.loading') : t('growingFees.recordPayment')}
            </button>
          </div>
        </form>
      </div>
    ...
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-4 px-5 py-4 bg-gray-50 border-b border-gray-100">
        ...
        <div className="shrink-0 text-sm text-gray-500">
          {entries.length} {t('growingFees.batches', { count: entries.length })}
        </div>
      </div>
        ...
        <table className="w-full text-sm min-w-[780px]">
          <thead>
            <tr className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider border-b border-gray-100">
              <th className="px-5 py-2.5">{t('growingFees.batchLabel')}</th>
              <th className="px-5 py-2.5 text-right">{t('batches.fcr')}</th>
              <th className="px-5 py-2.5">{t('growingFees.fcrTier')}</th>
              <th className="px-5 py-2.5 text-right">{t('growingFees.ratePerKg')}</th>
              <th className="px-5 py-2.5 text-right">{t('growingFees.grossFee')}</th>
              <th className="px-5 py-2.5 text-right">{t('growingFees.advances')}</th>
            ...
            {entries.map(entry => (
              <tr key={entry.id} className="hover:bg-gray-50/60 transition">
                <td className="px-5 py-3 text-gray-500 whitespace-nowrap">
                  {entry.batch_start_date ? formatDate(entry.batch_start_date, i18n.language) : '—'}
                </td>
      ...
      <div className="flex items-center justify-between gap-4 px-5 py-3 border-t border-gray-100 bg-gray-50/50">
        <div className="text-sm">
          <span className="text-gray-500">{t('growingFees.outstanding')}: </span>
          <span className={`font-bold ${outstanding > 0 ? 'text-red-600' : 'text-green-600'}`}>
            {outstanding > 0 ? formatCurrency(outstanding) : t('growingFees.fullyPaid')}
          </span>
        </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">{t('growingFees.title')}</h1>
          <p className="text-sm text-gray-500 mt-0.5">{t('growingFees.subtitle')}</p>
        </div>
        {canEdit && (
          <div className="flex items-center gap-2">
            ...
            >
              <span className="text-base leading-none">+</span> {t('growingFees.recordPayment')}
            </button>
          </div>
        )}
      </div>
        ...
        <div className="mb-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-3">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">{t('growingFees.totalGrossFees')}</p>
              <p className="text-2xl font-bold text-gray-800 mt-1">{formatCurrency(totalGrossFees)}</p>
              <p className="text-xs text-gray-400 mt-0.5">{t('growingFees.fromClosedBatches')}</p>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-amber-200 shadow-sm px-5 py-4">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">{t('growingFees.totalAdvances')}</p>
              <p className="text-2xl font-bold text-amber-600 mt-1">{formatCurrency(totalAllAdvances)}</p>
              <p className="text-xs text-gray-400 mt-0.5">{t('growingFees.allTimeAdvances')}</p>
            </div>

            {/* Card 3: Post-close Paid */}
            <div className="bg-white rounded-2xl border border-green-200 shadow-sm px-5 py-4">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">{t('growingFees.postClosePaid')}</p>
              <p className="text-2xl font-bold text-green-600 mt-1">{formatCurrency(totalPostClosePaid)}</p>
              <p className="text-xs text-gray-400 mt-0.5">{t('growingFees.paymentsAfterClose')}</p>
            </div>

            {/* Card 4: Outstanding */}
            <div className={`bg-white rounded-2xl border shadow-sm px-5 py-4 ${totalPending > 0 ? 'border-red-200' : 'border-gray-100'}`}>
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">{t('growingFees.outstanding')}</p>
              <p className={`text-2xl font-bold mt-1 ${totalPending > 0 ? 'text-red-600' : 'text-green-600'}`}>
                {formatCurrency(totalPending)}
              </p>
              <p className="text-xs text-gray-400 mt-0.5">{t('growingFees.balanceDueOnClosed')}</p>
            </div>
          </div>
          ...
          {activeAdvancesTotal > 0 && (
            <div className="flex flex-wrap items-center gap-1.5 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-sm">
              <span className="font-semibold text-amber-800">{t('growingFees.activeBatchAdvances')}: {formatCurrency(activeAdvancesTotal)}</span>
              <span className="text-amber-600">— {t('growingFees.activeBatchAdvancesNote')}</span>
            </div>
          )}
        </div>
      )}
      ...
      {!loading && activeBatchFarms.length > 0 && (
        <div className="bg-white border border-green-100 rounded-2xl shadow-sm overflow-hidden mb-6">
          <div className="flex items-center justify-between px-5 py-4 bg-green-50 border-b border-green-100">
            <div>
              <p className="text-sm font-bold text-green-800">{t('growingFees.activeBatchAdvances')}</p>
              <p className="text-xs text-green-600 mt-0.5">{t('growingFees.activeBatchAdvancesDesc')}</p>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[560px]">
              <thead>
                <tr className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider border-b border-gray-100">
                  <th className="px-5 py-2.5">{t('reports.farm')}</th>
                  <th className="px-5 py-2.5">{t('farms.owner')}</th>
                  <th className="px-5 py-2.5">{t('growingFees.activeBatch')}</th>
                  <th className="px-5 py-2.5 text-right">{t('growingFees.advancesGiven')}</th>
                  ...
                  return (
                    <tr key={af.farm_id} className="hover:bg-gray-50/60 transition">
                      ...
                      <td className="px-5 py-3 text-gray-500">
                        {newestBatch ? `${t('growingFees.batchLabel')} ${formatDate(newestBatch.start_date, i18n.language)} — ${t('growingFees.day')} ${day}` : '—'}
                      </td>
                      <td className="px-5 py-3 text-right font-semibold text-amber-700">
                        {totalAdv > 0 ? formatCurrency(totalAdv) : <span className="text-gray-300 font-normal">{t('growingFees.noneYet')}</span>}
                      </td>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <div className="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
            {[
              ...
              { key: 'Pending', label: t('growingFees.status.pending') },
              { key: 'Partial', label: t('growingFees.status.partial') },
              { key: 'Paid',    label: t('growingFees.status.paid') },
            ].map(s => (
              <button
                key={s.key}
          ...
          {ledgerFarms.length > 1 && (
            ...
            >
              <option value="">{t('reports.allFarms')}</option>
              {ledgerFarms.map(f => (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          >
            {t('growingFees.tryAgain')}
          </button>
        </div>
      ) : ledger.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-gray-400">
          <span className="text-5xl mb-3">🐔</span>
          <p className="text-sm font-medium text-gray-500">{t('growingFees.noGrowingFees')}</p>
          <p className="text-xs mt-1 text-gray-400">{t('growingFees.noFeesHint')}</p>
        </div>
      ) : farmGroups.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-gray-400">
          <span className="text-4xl mb-2">🔍</span>
          <p className="text-sm">{t('growingFees.noMatchFilters')}</p>
          <button
